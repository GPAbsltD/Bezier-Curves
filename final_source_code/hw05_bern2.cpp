...
int main(int argc, char **argv) {
    ...
    for (int i = start_i; i <= end_i; i++) {  
        ...
        currentX = pow((1-t), 3)*pointX[0] + 3*t*pow((1-t), 2)*pointX[1] +
                   3*pow(t, 2)*(1-t)*pointX[2] + pow(t, 3)*pointX[3];
        currentY = pow((1-t), 3)*pointY[0] + 3*t*pow((1-t), 2)*pointY[1] +
                   3*pow(t, 2)*(1-t)*pointY[2] + pow(t, 3)*pointY[3];
